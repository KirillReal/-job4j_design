...
public class Zip {
    public void packFiles(List<Path> sources, File target) throws IOException {
        try (ZipOutputStream zin =
            ...
            while (it.hasNext()) {
                ...
                try (BufferedInputStream tar = new BufferedInputStream(new FileInputStream(it
                        .next().toFile()))) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        argZip.setValue();
        List<Path> list = SimpleSearch.search(Paths
                .get(argZip.getValue("d")), (argZip.getValue("e")));
