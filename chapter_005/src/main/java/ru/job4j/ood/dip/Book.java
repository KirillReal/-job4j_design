...
//Наш высокоуровневый класс ElectricPowerSwitch напрямую зависит от низкоуровневого класса LightBulb.
//класс LightBulb жестко запрограммирован в ElectricPowerSwitch.
// Но выключатель не должен быть привязан к лампочке. Он должен иметь возможность включать и выключать другие приборы и устройства, например вентилятор, кондиционер
// и тд. Теперь представьте, какие модификации нам потребуются в классе ElectricPowerSwitch каждый раз, когда мы добавляем новое устройство или устройство.
// Мы можем сделать вывод, что наш дизайн ошибочен, и нам необходимо пересмотреть его, следуя принципу инверсии зависимостей.
