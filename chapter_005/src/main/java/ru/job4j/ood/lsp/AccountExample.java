...
class MicroAccount extends AccountExample {
    ...
    // Подкласс MicroAccount добавляет дополнительное предусловие, то есть усиливает его, что недопустимо
    @Override
    public  void SetCapital(int money) throws Exception {
        if (money < 0)
            throw new Exception("Нельзя положить на счет меньше 0");
    ...
    // В итоге мы приходим к нарушению принципа Лисков - Постусловия не могут быть ослаблены в подклассе
    @Override
    public double GetInterest(double sum, int month, int rate) throws Exception {
        if (sum < 0 || month > 12 || month < 1 || rate < 0)
            throw new Exception("Некорректные данные");
    ...
    // А вот производный класс MicroAccount, переопределяя свойство Capital, этого уже не гарантирует.
